...
export function interpretPlot(plotSpec) {
  ...
  try {
    ...
    const plotType = raw === "curve" ? "curve2d" : raw === "2d" ? "curve2d" : raw;
      ...
      return {
        model: {
          ...
          layout: {
            ...
            yaxis: Array.isArray(ps.yRange) && ps.yRange.length === 2
              ? { range: ps.yRange }
              : undefined,
    ...
    if (plotType === "surface" || plotType === "contour") {
      ...
      const overlay2D = plotType === "contour" && overlayPoints.length
        ? [pointTrace2D(overlayPoints)]
        : [];
      
      const overlay3D = plotType === "surface" && overlayPoints.length && funcs.length
        ? (() => {
            let f0;
            try {
              f0 = compileF2(funcs[0].expression);
            } catch {
              return [];
            }
            return [{
